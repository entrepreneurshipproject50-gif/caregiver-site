...
// Parse JSON bodies (for fetch("/api/messages", { body: JSON.stringify(...) }))
...
— Your Name`,
  ...
  try {
    ...
    // Simple response page after submit
    res.send(`
      <!DOCTYPE html>
      <html>
      <head>
...
app.post("/quiz", (req, res) => {
  ...
  // Determine stage based on recent_changes (from your logic)
  let stageNumber = 1;
  ...
  // Prepare CSV row (ISO date + all answers + stage)
  ...
  fs.appendFile(csvPath, row, (err) => {
    ...
    // Return a simple results page styled to match your theme
    res.send(`
      <!DOCTYPE html>
      <html lang="en">
      <head>
